<!-- ANIMATED GITHUB PROFILE README for @Vin0210 (Elvin Ramos) -->
<!-- Copy ALL of this file into https://github.com/Vin0210/Vin0210/blob/main/README.md -->
<!-- Replace YOUR_LINKEDIN_URL, YOUR_PORTFOLIO_URL, YOUR_EMAIL at the bottom -->

<!-- ===== 1. ANIMATED HEADER WAVE ===== -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0D9488&height=220&section=header&text=Elvin%20Ramos&fontSize=55&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=Full-Stack%20Developer%20%7C%20Laravel%20%26%20React&descSize=18&descAlignY=58&descAlign=center" alt="header" />

<!-- ===== 2. TYPING ANIMATION ===== -->
<div align="center">
  <a href="https://git.io/typing-svg">
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&duration=3000&pause=1000&color=0D9488&center=true&vCenter=true&width=650&lines=Building+School+Management+Systems;Full-Stack+Laravel+%2B+React+Developer;Shipping+DentalOS+%26+SmashPoint;Designing+Practical+Web+Apps;Always+Learning+New+Things" alt="Typing SVG" />
  </a>
  <br />

<!-- ===== 3. LIVE COUNTERS (auto-update = feels animated) ===== -->
  <a href="https://github.com/Vin0210">
    <img src="https://komarev.com/ghpvc/?username=Vin0210&label=Profile%20Views&color=0D9488&style=for-the-badge" alt="Profile Views" />
  </a>
  <a href="https://github.com/Vin0210?tab=followers">
    <img src="https://img.shields.io/github/followers/Vin0210?label=Followers&style=for-the-badge&color=0D9488" alt="GitHub Followers" />
  </a>
  <img src="https://img.shields.io/badge/Focus-Full--Stack-teal?style=for-the-badge&logo=codecrafters&logoColor=white&color=0D9488" alt="Focus" />
</div>

<br />

<!-- ===== 4. ABOUT + ANIMATED CODING GIF ===== -->
## 👨‍💻 About Me

<img align="right" width="320" alt="coding gif" src="https://media.giphy.com/media/qgQUggAC3Pfv687qPC/giphy.gif" />

Full-stack developer focused on building **practical, maintainable web applications**.

- 🔭 Currently maintaining a **school management system in production** (RMMC)
- 🚀 Shipping **DentalOS** — clinic management: patients, appointments, clinical records, billing, inventory
- 🛠️ Experienced with **Laravel, PHP, React, JavaScript, MySQL**
- 🧠 Into **clean UI, system architecture, database design, interactive experiences**
- 🌱 Learning **Advanced React, API architecture, modern web animations**
- 📫 Open for **freelance, collaborations & junior dev roles**

<br clear="both" />

<!-- ===== 5. TECH STACK (animated on hover via skillicons) ===== -->
## 🛠️ Tech Stack

<div align="center">
  <img src="https://skillicons.dev/icons?i=php,laravel,react,vite,js,mysql,supabase,mongodb,bootstrap,tailwind,jquery,git,github,vscode,figma&perline=8" alt="Tech Stack" />
  <br />
  <img src="https://img.shields.io/badge/Laravel-Backend-0D9488?style=for-the-badge&logo=laravel&logoColor=white" alt="Laravel" />
  <img src="https://img.shields.io/badge/React-Frontend-0D9488?style=for-the-badge&logo=react&logoColor=white" alt="React" />
  <img src="https://img.shields.io/badge/MySQL-Database-0D9488?style=for-the-badge&logo=mysql&logoColor=white" alt="MySQL" />
  <img src="https://img.shields.io/badge/Supabase-Backend-0D9488?style=for-the-badge&logo=supabase&logoColor=white" alt="Supabase" />
</div>

<!-- ===== 6. ANIMATED STATS ===== -->
## 📊 GitHub Statistics

<div align="center">
  <img height="180" src="https://github-readme-stats.vercel.app/api?username=Vin0210&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=0D9488&icon_color=0D9488&text_color=C9D1D9" alt="GitHub Stats" />
  <img height="180" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Vin0210&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=0D9488&text_color=C9D1D9" alt="Top Languages" />
  <br />
  <img src="https://streak-stats.demolab.com?user=Vin0210&theme=tokyonight&hide_border=true&background=0D1117&ring=0D9488&fire=0D9488&currStreakLabel=0D9488&sideNums=C9D1D9&currStreakNum=C9D1D9&dates=C9D1D9" alt="GitHub Streak" />
  <br /><br />
  <!-- Activity graph: bars grow every day -->
  <img width="100%" src="https://github-readme-activity-graph.vercel.app/graph?username=Vin0210&bg_color=0D1117&color=5BC0BE&line=0D9488&point=FFFFFF&area=true&hide_border=true" alt="Activity Graph" />
  <br /><br />
  <img src="https://github-profile-trophy.vercel.app/?username=Vin0210&theme=tokyonight&no-frame=true&no-bg=true&margin-w=4&row=1&column=6" alt="Trophies" />
</div>

<!-- ===== 7. FEATURED PROJECTS ===== -->
## 🚀 Featured Projects

| Project | What it is | Stack | Links |
|---|---|---|---|
| ☕ **Alegre × Good Habits** | Editorial e-commerce for coffee + thrift shop, unified cart, PayMongo checkout | React 19, Vite, Supabase | [Live](https://alegrexgoodhabits.pages.dev/) · [Code](https://github.com/Vin0210/Coffee) |
| 🦷 **DentalOS** | Clinic management: patients, appointments, odontogram chart, billing, inventory | React, Laravel, MySQL | [Live](https://dentalos-frontend.vercel.app/) · [Code](https://github.com/Vin0210/DentalOS) |
| 🎓 **RMMC System** | School management in production: admissions, enrollment, finance, grading | Laravel, MySQL, jQuery | [Live](https://rmmcmain.com/) |
| 🏓 **SmashPoint** | Pickleball court booking with conflict-proof time slots | Laravel, React, MySQL | [Live](https://smashpoint.whf.bz/) · [Code](https://github.com/Vin0210/SmashPoint) |
| 🚗 **DriveRent** | Car rental + fleet dashboard (in progress) | Laravel, React, MySQL | [Code](https://github.com/Vin0210/Car-Rental) |
| ⚡ **Pokémon Web** | Pokédex + team builder + battle sim | React, CSS | [Live](https://pokemonhehe.netlify.app/) · [Code](https://github.com/Vin0210/Pokemon) |

<!-- ===== 8. SNAKE ANIMATION (eats your contributions) ===== -->
## 🐍 Contribution Snake

<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/Vin0210/Vin0210/output/github-contribution-grid-snake-dark.svg" />
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/Vin0210/Vin0210/output/github-contribution-grid-snake.svg" />
    <img alt="snake eating contributions" src="https://raw.githubusercontent.com/Vin0210/Vin0210/output/github-contribution-grid-snake-dark.svg" />
  </picture>
  <br />
  <i>⚠️ Snake appears after you add the workflow below (one-time setup, 2 min). It updates daily automatically.</i>
</div>

<!-- ===== 9. CONNECT ===== -->
## 🤝 Connect

<div align="center">
  <a href="YOUR_LINKEDIN_URL">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
  <a href="YOUR_PORTFOLIO_URL">
    <img src="https://img.shields.io/badge/Portfolio-0D9488?style=for-the-badge&logo=vercel&logoColor=white" alt="Portfolio" />
  </a>
  <a href="mailto:YOUR_EMAIL">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
  </a>
</div>

<!-- ===== 10. ANIMATED FOOTER WAVE ===== -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0D9488&height=120&section=footer" alt="footer" />

<!--
═══════════════════════════════════════════════════
SNAKE SETUP (one time, makes section 8 work):
1. In repo Vin0210/Vin0210, create file: .github/workflows/snake.yml
   (copy contents from snake.yml I gave you)
2. Go to repo Settings > Actions > General > Workflow permissions
   → check "Read and write permissions" → Save
3. Go to Actions tab > run "Generate Snake" once manually.
   A branch called "output" will be created with the animated SVGs.
4. Done. Snake updates every 24h automatically.
═══════════════════════════════════════════════════
-->
